...
'''
calculo_autoconsumido_v10_db.py
30-Agosto-2022
Versión para escribir en la base de datos del proyecto

Para isertar los datos de consumo en la Base de datos es necesario que en cada 
"posicion" de reading_register_ se incluya también "id_user"

Para conectarse a la base de datos es necesario que las credenciales esten en
fichero: "config_autoconsumido.ini"

Puede ejecutarse de forma autónoma a la actualización de emonCMS
  * Tiene comentada la escritura en emonCMS
  * Tiene comentada la actualización de reading_register.txt
  
Si se cree conveniente se puede descomentar las líneas anteriores y ejecutar solo un script
...
Para isertar los datos de consumo en la Base de datos es necesario que en la 
"posicion" de reading_register_ se incluya también "id_user"
Mejora: extraern id_user de una tabla de la base de datos a partir del del key y el name
...
def db_tx(position):
    ...
    exportedF = round(reading_register_[position]["exported"],3)
    importedF = round(reading_register_[position]["imported"],3)
    autoconsumedF = round(reading_register_[position]["autoconsumed"],3)
    id_userI = reading_register_[position]["id_user"]
    
    #Correcciones horarias 
    timeI = reading_register_[position]["ultima"].split("+")[0]
    # emonCms muestra el consumo de una hora en el inicio de la hora (+1)
    zoneI = int(reading_register_[position]["ultima"].split("+")[1].split(":")[0]) - 1
    enerTimeD = isoformatD(timeI) - timedelta(hours=zoneI)
    ...
    # Obtenemos el último registro de la tabla en base de datos
    sql = "select max(id_user_counter) from cometa.user_counter";
    cursor.execute(sql);
    rowData = cursor.fetchone();
    numMaxGeneratorData = rowData[0];
    newIndexGeneratorData = numMaxGeneratorData;
    newIndexGeneratorData = int(numMaxGeneratorData) + 1;

    '''
    Formato del INSERT:
    INSERT INTO cometa.user_counter (id_user_counter, id_user, timestamp, exported_energy, imported_energy, self_consumed_energy) VALUES (12, 14, '2022-08-30 06:07:00', 1.1, 1.579, 1.1)
Row Data: ('ES0031104616159079EA0F', 8)
    '''
    sentenciaInsert = "INSERT INTO cometa.user_counter (id_user_counter, id_user, timestamp, exported_energy, imported_energy, self_consumed_energy) VALUES ({}, {}, \'{}\', {}, {}, {})";
    sentenciaInsert1 =sentenciaInsert.format(newIndexGeneratorData, id_userI, enerTimeD, exportedF, importedF, autoconsumedF)
    
    cursor.execute(sentenciaInsert1)
    cursor.close()
    conexion.commit();    
    
    # print(sentenciaInsert1)
    logging.debug("++++ " + sentenciaInsert1)
